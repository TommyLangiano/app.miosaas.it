...
export default function ModificaFornitorePage() {
  ...
  const [form, setForm] = useState<FornitoreForm>({ forma_giuridica: 'PG', so_diversa: false });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const { data } = await axios.get(`/api/tenants/fornitori/${id}`, { headers });
        if (!active) return;
        const record = { ...(data.data as FornitoreForm) };
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setForm((prev) => ({ ...prev, [name as keyof FornitoreForm]: value } as FornitoreForm));
  };
  ...
  return (
    <MC title={`Modifica Fornitore #${id}`}>
      {loading ? (
        ...
        <form onSubmit={handleSubmit} noValidate autoComplete="off">
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12 }}>
              <SC title="Indirizzo sede legale">
              ...
              <SC title="Dati amministrativi">
                <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', md: '1fr 1fr 1fr 1fr' } }}>
                  ...
                  <FormControl fullWidth>
                    <InputLabel id="aliquota-iva-fornitori-label" shrink>Aliquota IVA predefinita</InputLabel>
                    <Select
                      labelId="aliquota-iva-fornitori-label"
