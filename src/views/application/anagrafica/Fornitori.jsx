...
export default function Fornitori() {
  ...
  const { data, isLoading } = useSWR(['api/tenants/fornitori', { params: query ? { q: query } : undefined }], fetcher);
  const rows = useMemo(() => (data?.data || []).map((r, i) => ({ id: r.id ?? i, ...r })), [data]);

  const getDisplayName = (row) => (row?.ragione_sociale || '').trim() || (row?.nome || '') + (row?.cognome ? ` ${row.cognome}` : '') || '—';
  ...
  const getFormaLabel = (row) => ((row?.forma_giuridica || '').toUpperCase() === 'PF' ? 'Privato' : row?.forma_giuridica_dettaglio || 'Azienda');
      ...
      <Grid size={12}>
        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
          <TextField
            ...
            placeholder="Cerca fornitori (ragione sociale, P.IVA, email...)"
            value={query}
