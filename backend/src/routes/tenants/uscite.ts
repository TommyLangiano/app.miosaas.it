...
// GET /api/tenants/uscite?commessa_id=123
...
router.post('/', async (req: AuthedTenantRequest, res: Response): Promise<void> => {
  try {
    ...
    const required = ['fornitore','tipologia','numero_fattura','emissione_fattura','importo_totale','aliquota_iva','imponibile','iva','data_pagamento','stato_uscita','commessa_id'];
...
router.put('/:id', async (req: AuthedTenantRequest, res: Response): Promise<void> => {
  try {
    ...
    const allowed = [
      'fornitore','tipologia','numero_fattura','emissione_fattura','importo_totale','aliquota_iva','imponibile','iva','data_pagamento','modalita_pagamento','banca_emissione','numero_conto','stato_uscita','commessa_id','updated_by'
    ];
