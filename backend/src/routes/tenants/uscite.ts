...
router.post('/', async (req: AuthedTenantRequest, res: Response): Promise<void> => {
  try {
    ...
    if (error instanceof Error) {
      ...
      if (errorMessage.includes('uniq_uscite_company_invoice') || errorMessage.includes('duplicate key value violates unique constraint')) {
        console.log('🔍 Backend: Rilevato numero fattura duplicato, restituisco errore 400');
        res.status(400).json({ 
          status: 'error', 
    ...
    console.log('🔍 Backend: Errore generico, restituisco 500');
